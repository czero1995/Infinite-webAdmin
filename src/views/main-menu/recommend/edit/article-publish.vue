                ...
                <Card>
                    <Form :label-width="80">
                        <FormItem label="标题" :error="articleError">
                            <Input v-model="dataList.title" @on-blur="handleArticletitleBlur" icon="android-list"/>
                        </FormItem>
                        <div class="article-link-con">
                           <div class="mainContainer">
                                <input class="file-input" type="file" id="select" ref="imageref" @change="fileChange($event)"/>
                                <img  class="show_img" :src="dataList.post" alt="">
                           </div>
                           
                        </div>
                    </Form>
                    <div class="margin-top-20">
                        <textarea id="articleEditor" v-model="dataList.content"></textarea>
                    </div>
                    <div class="publish_box" @click="handlePreview">编辑</div>
                </Card>
            </Col>
...
export default {
    ...
    methods: {
        handleArticletitleBlur () {
            if (this.dataList.title.length !== 0) {
                // this.articleError = '';
        ...
        handlePreview () {
            ...
            this.$http.post(`${path}recommend/update`,{
                id:this.$route.query.id,
                title:that.dataList.title,
                post:that.dataList.post,
                content:that.dataList.content
           }).then(res => {
                console.log('biaji',res)
            });
        },
        getData(){
            console.log('id',this.$route.query.id)
            this.$http.get(`${path}recommend/detail?id=${this.$route.query.id}`,{
           }).then(res => {
        ...
        getToken(){
             this.$http.post('${path}api/uptoken').then(res => {
        ...
        fileChange(){
            ...
            let complete = (res) => {
                ...
                var key = res.key
                var fsize = res.size
                this.dataList.post = `${this.qiniuDomain}${key}`
        ...
        tinymce.init({
            ...
            setup: function (editor) {
                ...
                editor.on('keydown', function (e) {
                    that.dataList.content = tinymce.get('articleEditor').getContent()
